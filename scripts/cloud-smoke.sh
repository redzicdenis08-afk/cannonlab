...
MIN_EMBEDDED_PAYLOAD_EXPLOSIONS="${CANNONLAB_MIN_EMBEDDED_PAYLOAD_EXPLOSIONS:-}"
MAX_UNEMBEDDED_WATER_EXPLOSIONS="${CANNONLAB_MAX_UNEMBEDDED_WATER_EXPLOSIONS:-}"
MIN_CONTIGUOUS_LAYERS_BEFORE_FIRST_REGEN="${CANNONLAB_MIN_CONTIGUOUS_LAYERS_BEFORE_FIRST_REGEN:-}"
REQUIRE_ALL_LAYERS_BEFORE_FIRST_REGEN="${CANNONLAB_REQUIRE_ALL_LAYERS_BEFORE_FIRST_REGEN:-false}"
MAX_SELF_DAMAGE_BLOCKS="${CANNONLAB_MAX_SELF_DAMAGE_BLOCKS:-}"
MAX_CANNON_UNEXPECTED_BLOCKS="${CANNONLAB_MAX_CANNON_UNEXPECTED_BLOCKS:-}"
REQUIRE_REGEN="${CANNONLAB_REQUIRE_REGEN:-false}"
MIN_REGEN_RESTORED="${CANNONLAB_MIN_REGEN_RESTORED:-1}"
MIN_DURABILITY_HITS="${CANNONLAB_MIN_DURABILITY_HITS:-}"
...
REQUIRE_CUMULATIVE_CANNON="${CANNONLAB_REQUIRE_CUMULATIVE_CANNON:-false}"
...
if [[ -n "$MIN_EMBEDDED_PAYLOAD_EXPLOSIONS" ]]; then
  ASSERT_ARGS+=(--min-embedded-payload-explosions "$MIN_EMBEDDED_PAYLOAD_EXPLOSIONS")
fi
if [[ -n "$MAX_UNEMBEDDED_WATER_EXPLOSIONS" ]]; then
  ASSERT_ARGS+=(--max-unembedded-water-explosions "$MAX_UNEMBEDDED_WATER_EXPLOSIONS")
fi
if [[ -n "$MIN_CONTIGUOUS_LAYERS_BEFORE_FIRST_REGEN" ]]; then
  ASSERT_ARGS+=(--min-contiguous-layers-before-first-regen "$MIN_CONTIGUOUS_LAYERS_BEFORE_FIRST_REGEN")
fi
case "${REQUIRE_ALL_LAYERS_BEFORE_FIRST_REGEN,,}" in
  1|true|yes) ASSERT_ARGS+=(--require-all-layers-before-first-regen) ;;
  0|false|no) ;;
  *) echo "Invalid CANNONLAB_REQUIRE_ALL_LAYERS_BEFORE_FIRST_REGEN=$REQUIRE_ALL_LAYERS_BEFORE_FIRST_REGEN" >&2; exit 1 ;;
esac
if [[ -n "$MAX_SELF_DAMAGE_BLOCKS" ]]; then
  ASSERT_ARGS+=(--max-self-damage-blocks "$MAX_SELF_DAMAGE_BLOCKS")
fi
if [[ -n "$MAX_CANNON_UNEXPECTED_BLOCKS" ]]; then
  ASSERT_ARGS+=(--max-cannon-unexpected-blocks "$MAX_CANNON_UNEXPECTED_BLOCKS")
fi
if [[ -n "$MIN_DURABILITY_HITS" ]]; then
  ASSERT_ARGS+=(--min-durability-hits "$MIN_DURABILITY_HITS")
fi
if [[ -n "$MIN_DURABILITY_BREAKS" ]]; then
  ASSERT_ARGS+=(--min-durability-breaks "$MIN_DURABILITY_BREAKS")
fi
if [[ -n "$MAX_COMPANION_MISSING" ]]; then
  ASSERT_ARGS+=(--max-companion-missing "$MAX_COMPANION_MISSING")
fi
if [[ -n "$MIN_COMPANION_RESTORED" ]]; then
  ASSERT_ARGS+=(--min-companion-restored "$MIN_COMPANION_RESTORED")
fi
case "${REQUIRE_CUMULATIVE_CANNON,,}" in
  1|true|yes) ASSERT_ARGS+=(--require-cumulative-cannon) ;;
  0|false|no) ;;
  *) echo "Invalid CANNONLAB_REQUIRE_CUMULATIVE_CANNON=$REQUIRE_CUMULATIVE_CANNON" >&2; exit 1 ;;
esac
