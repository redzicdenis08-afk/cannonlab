...
def run_json(args: list[str]) -> tuple[int, dict[str, Any]]:
    result = subprocess.run(args, cwd=ROOT, capture_output=True, text=True, check=False, timeout=180)
    try:
        payload = json.loads(result.stdout)
    except json.JSONDecodeError:
        payload = {"status": "ERROR", "error": result.stderr[-2000:] or result.stdout[-2000:]}
    return result.returncode, payload


def classify(dispensers: int, height: int, functional_height: int) -> list[str]:
    labels: list[str] = []
    if dispensers == 0:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Batch audit and structurally compare a private cannon corpus")
    parser.add_argument("directory", type=Path)
    parser.add_argument("--chunk-limit", type=int, default=160)
    parser.add_argument("--json-out", type=Path)
    args = parser.parse_args()

    files = sorted(path for path in args.directory.rglob("*") if path.is_file() and path.suffix.lower() in EXTENSIONS)
    if not files:
    ...
    for path in files:
        ...
        functional_height = int(bounds.get("height", 0)) if isinstance(bounds, dict) else 0
        ...
        designs.append({
            ...
            "shape_labels": classify(dispensers, int(dimensions.get("height", 0)), functional_height),
            "errors": audit.get("errors", []),
            "warnings": audit.get("warnings", []),
            "truth_boundary": "Shape labels are structural only. Runtime traces are required before naming charge, hammer, booster, nuke, rev-worm or OSRB roles.",
        })
