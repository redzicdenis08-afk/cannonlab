...
const OUTPUT_DIR = path.resolve(process.env.PHASELAB_OUTPUT || 'output-private-adaptive-down')
const STEP = 0.25
const TOP_Y = 96.0
const TARGET_Y = 75.5
...
const report = {
  ...
  scope: 'authorized exact private Sakura/Factions adaptive downward phase with normal remount validation',
...
async function setupFactions (phaseBot, victimBot, attackerBot) {
  ...
  await commandExpect(phaseBot, '/stacklab claimset Attackers 0 0', /STACKLAB CLAIM SET .*"actual_tag":"Attackers".*"verified":true/, 6000)
  ...
  report.claimWitness = 'Mount in attacker claim, then transfer roof chunk to Victims before movement.'
  record('claims_verified', { witness: report.claimWitness })
}

async function buildVerticalCourse (phaseBot) {
  await command(phaseBot, '/fill -8 70 -8 8 106 8 minecraft:air', 700)
  await command(phaseBot, '/fill -7 70 -7 7 80 7 minecraft:obsidian', 250)
  await command(phaseBot, '/fill -5 72 -5 5 79 5 minecraft:air', 200)
  for (let y = 80; y <= 95; y++) {
    const material = PALETTE[(95 - y) % PALETTE.length]
    await command(phaseBot, `/fill -5 ${y} -5 5 ${y} 5 ${material}`, 25)
  }
  await command(phaseBot, `/summon minecraft:item 0.5 74.5 0.5 {Item:{id:"minecraft:netherite_block",count:1}}`, 150)
  record('course_built', {
    topY: TOP_Y,
    targetY: TARGET_Y,
    layers: 16,
...
async function resetAttacker (phaseBot, attackerBot, vehicle) {
  ...
  if (vehicle === 'boat') {
    await command(phaseBot, '/tp AttackerBot 0.5 98 4.5 180 75', 350)
  } else {
    await command(phaseBot, '/tp AttackerBot 0.5 98 4.0 180 75', 350)
  }
}
...
async function remountAfterSeparation (phaseBot, attackerBot, vehicle, existingEntity) {
  ...
  await sleep(220)
  const visible = Object.values(attackerBot.entities).find(entity => entity.id === existingEntity.id) || await waitForVehicle(attackerBot, vehicle, 2000)
  if (!visible) throw new Error(`Vehicle vanished during adaptive remount: ${response}`)
  attackerBot.vehicle = visible
  attackerBot.entity.vehicle = visible
  if (!visible.passengers.includes(attackerBot.entity)) visible.passengers.push(attackerBot.entity)
  return response
...
async function sendDownSegment (bot, targetY, onGround) {
  ...
  for (let y = vehicle.position.y - STEP; y >= targetY - 1e-9; y -= STEP) {
    const clamped = Math.max(y, targetY)
    // Match Fabric 4.8: vehicle-only local movement, one packet per tick.
    vehicle.position.set(x, clamped, z)
    bot._client.write('vehicle_move', { x, y: clamped, z, yaw: vehicle.yaw || 0, pitch: 90, onGround: false })
    packets++
    await sleep(50)
    if (clamped <= targetY + 1e-9) break
  }
  return packets
}
...
async function runTrial (phaseBot, attackerBot, trial, run) {
  ...
  await commandExpect(phaseBot, '/stacklab claimset Victims 0 0', /STACKLAB CLAIM SET .*"actual_tag":"Victims".*"verified":true/, 6000)
  const start = { x: mountedVehicle.position.x, y: mountedVehicle.position.y, z: mountedVehicle.position.z }
  await command(phaseBot, `/summon minecraft:item ${start.x} 74.5 ${start.z} {Item:{id:"minecraft:netherite_block",count:1}}`, 100)

  const corrections = captureCorrections(attackerBot, mountedVehicle.id)
  const pulses = []
  if (trial.pulse === 'pre') {
    pulses.push(await realPulse(phaseBot, attackerBot, trial.vehicle, mountedVehicle))
  }

  let acceptedY = attackerBot.vehicle.position.y
  let packetCount = 0
  let segment = 0
  const segmentWitnesses = []
  const started = Date.now()

  while (acceptedY > TARGET_Y + 1e-9 && attackerBot.vehicle) {
    segment++
    const targetY = Math.max(TARGET_Y, acceptedY - trial.segmentLength)
    packetCount += await sendDownSegment(attackerBot, targetY, trial.onGround)
    await sleep(trial.pauseMs)

    const x = attackerBot.vehicle.position.x
    const z = attackerBot.vehicle.position.z
    const playerAccepted = await serverWitness(phaseBot, verticalPlayerSelector(x, targetY, z), `V_PLAYER_${trial.id}_${run}_${segment}`)
    const vehicleAccepted = await serverWitness(phaseBot, verticalVehicleSelector(trial.vehicle, x, targetY, z), `V_VEHICLE_${trial.id}_${run}_${segment}`)
    const witness = { segment, fromY: acceptedY, targetY, playerAccepted, vehicleAccepted, corrections: corrections.events.length, remounted: false }
    segmentWitnesses.push(witness)
    record('vertical_segment', { trial: trial.id, run, ...witness })
    if (corrections.events.length > 0 || !vehicleAccepted) break
    if (!playerAccepted) {
      try {
        const response = await remountAfterSeparation(phaseBot, attackerBot, trial.vehicle, mountedVehicle)
        witness.remounted = true
        witness.remountResponse = response
        record('adaptive_down_remount', { trial: trial.id, run, segment, targetY, response })
      } catch (error) {
        witness.remountError = String(error.stack || error)
        record('adaptive_down_remount_error', { trial: trial.id, run, segment, targetY, error: witness.remountError })
        break
      }
    }
    acceptedY = targetY

    if (trial.pulse === 'segment' && acceptedY > TARGET_Y + 1e-9) {
      try {
        pulses.push(await realPulse(phaseBot, attackerBot, trial.vehicle, mountedVehicle))
      } catch (error) {
        pulses.push({ error: String(error.stack || error) })
        break
      }
    }
  }

  const x = attackerBot.vehicle ? attackerBot.vehicle.position.x : start.x
  const z = attackerBot.vehicle ? attackerBot.vehicle.position.z : start.z
  const playerMountedBelow = await serverWitness(phaseBot, verticalPlayerSelector(x, TARGET_Y, z, 4), `V_FINAL_PLAYER_${trial.id}_${run}`)
  const vehicleBelow = await serverWitness(phaseBot, verticalVehicleSelector(trial.vehicle, x, TARGET_Y, z, 4), `V_FINAL_VEHICLE_${trial.id}_${run}`)
  corrections.stop()

  let dismountResponse = null
  let dismountError = null
  try {
    dismountResponse = await commandExpect(phaseBot, '/stacklab vehicledismount AttackerBot', /STACKLAB VEHICLE DISMOUNT .*"mounted":false/, 6000)
    attackerBot.vehicle = null
    attackerBot.entity.vehicle = null
  } catch (error) {
  ...
  await sleep(1000)
  const playerDismountedBelow = await serverWitness(phaseBot, verticalPlayerSelector(x, TARGET_Y, z, 6), `V_FINAL_DISMOUNT_${trial.id}_${run}`)
  ...
  result.serverAuthoritativeSuccess = Boolean(
    acceptedY <= TARGET_Y + 1e-9 &&
    playerMountedBelow && vehicleBelow && playerDismountedBelow &&
    netheriteCount >= 1 && corrections.events.length === 0
  )
  report.trials.push(result)
  record('adaptive_down_remount_trial', result)
  return result
}
...
async function main () {
  ...
  const trialPlan = [
    { id: 'horse-adaptive-down-remount-075', vehicle: 'horse', pulse: 'none', onGround: false, segmentLength: 0.75, pauseMs: 350, repeats: 3 }
  ]
  ...
  try {
    ...
    for (const trial of trialPlan) {
      for (let run = 1; run <= trial.repeats; run++) {
        try {
          ...
          record('adaptive_down_remount_trial_error', failure)
        }
      }
    }
  } catch (error) {
    ...
    fs.writeFileSync(path.join(OUTPUT_DIR, 'private-stack-adaptive-down-remount-report.json'), JSON.stringify(report, null, 2))
    fs.writeFileSync(path.join(OUTPUT_DIR, 'private-stack-adaptive-down-remount-transcript.jsonl'), transcript.map(row => JSON.stringify(row)).join('\n') + '\n')
    for (const bot of bots) {
      try { bot.quit('PhaseLab adaptive down remount matrix complete') } catch {}
    }
  }
