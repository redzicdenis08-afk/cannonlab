...
async function main () {
  ...
  const trialPlan = []
  for (const vehicle of ['boat', 'horse']) {
    trialPlan.push(
      { id: `${vehicle}-direct-air-4`, vehicle, pulse: 'none', onGround: false, segmentLength: 4, pauseMs: 100, repeats: 2 },
      { id: `${vehicle}-direct-ground-4`, vehicle, pulse: 'none', onGround: true, segmentLength: 4, pauseMs: 100, repeats: 2 },
      { id: `${vehicle}-prepulse-air-4`, vehicle, pulse: 'pre', onGround: false, segmentLength: 4, pauseMs: 100, repeats: 2 },
      { id: `${vehicle}-segmentpulse-air-4`, vehicle, pulse: 'segment', onGround: false, segmentLength: 4, pauseMs: 100, repeats: 2 }
    )
  }
  ...
  try {
    ...
    for (const trial of trialPlan) {
      for (let run = 1; run <= trial.repeats; run++) {
        try {
          ...
          record('vertical_trial_error', failure)
        }
      }
    }
  } catch (error) {
    ...
    fs.writeFileSync(path.join(OUTPUT_DIR, 'private-stack-vertical-phase-report.json'), JSON.stringify(report, null, 2))
    fs.writeFileSync(path.join(OUTPUT_DIR, 'private-stack-vertical-phase-transcript.jsonl'), transcript.map(row => JSON.stringify(row)).join('\n') + '\n')
    for (const bot of bots) {
      try { bot.quit('PhaseLab vertical matrix complete') } catch {}
    }
  }
