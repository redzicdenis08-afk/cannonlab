...
const OUTPUT_DIR = path.resolve(process.env.PHASELAB_OUTPUT || 'output-private-adaptive-horse-speed')
const Y = 65
const STEP = 0.25
const PAUSE_MS = 350
...
const PROFILES = [
  { id: 'horse-p2-seg3', packetsPerTick: 2, segmentLength: 3.0 },
  { id: 'horse-p4-seg4', packetsPerTick: 4, segmentLength: 4.0 },
  { id: 'horse-p8-seg8', packetsPerTick: 8, segmentLength: 8.0 }
]
...
const report = {
  ...
  port: PORT,
  version: VERSION,
  scope: 'authorized exact private Sakura/Factions horse packet-rate speed matrix',
...
async function sendRange (bot, start, direction, fromDistance, toDistance, fixedY, yaw, correctionRef, packetsPerTick) {
  let packets = 0
  let distance = fromDistance + STEP
  while (distance <= toDistance + 1.0E-9 && bot.vehicle && !correctionRef.value) {
    for (let group = 0; group < packetsPerTick && distance <= toDistance + 1.0E-9; group++) {
...
async function runTrial (phaseBot, attackerBot, profile, run) {
  const vehicleName = 'horse'
  const directionName = 'east'
  const direction = { name: directionName, ...DIRECTIONS[directionName] }
  const vehicle = VEHICLES[vehicleName]
  const id = `${profile.id}-east-solid${THICKNESS}`
  const trial = { id, vehicle, direction, segmentLength: profile.segmentLength, packetsPerTick: profile.packetsPerTick }
  ...
  const result = {
    ...
    packetsPerTick: trial.packetsPerTick,
    pauseMs: PAUSE_MS,
    step: STEP,
  ...
  record('adaptive_horse_speed_trial', result)
  return result
}
  ...
  try {
    ...
    for (const profile of PROFILES) {
      for (let run = 1; run <= 1; run++) {
        try {
          await runTrial(phaseBot, attackerBot, profile, run)
        } catch (error) {
          const failure = {
            id: `${profile.id}-east-solid${THICKNESS}`,
            run,
            direction: 'east',
            vehicle: 'horse',
            packetsPerTick: profile.packetsPerTick,
            segmentLength: profile.segmentLength,
            serverAuthoritativeSuccess: false,
            error: String(error.stack || error)
          }
          report.trials.push(failure)
          record('adaptive_horse_speed_trial_error', failure)
        }
      }
    }
  } catch (error) {
    ...
    fs.writeFileSync(
      path.join(OUTPUT_DIR, 'private-stack-adaptive-horse-speed-report.json'),
      JSON.stringify(report, null, 2)
    )
    fs.writeFileSync(
      path.join(OUTPUT_DIR, 'private-stack-adaptive-horse-speed-transcript.jsonl'),
      transcript.map(row => JSON.stringify(row)).join('\n') + '\n'
    )
    for (const bot of bots) {
      try { bot.quit('PhaseLab adaptive horse speed matrix complete') } catch {}
    }
  }
