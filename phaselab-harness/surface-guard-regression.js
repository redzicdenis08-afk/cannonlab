...
async function runProbe (bot, name, cause, destination, expectedBlocked) {
  await resetEnemy(bot)
  const before = await snapshot(bot)
  await command(bot, `/surfaceguard probe ${cause} ${destination.x} ${destination.y} ${destination.z}`, 350)
  const after = await snapshot(bot)
  const blocked = after.ok && after.position.x < 0.0 && !after.protected && !after.collision
  const passed = expectedBlocked ? blocked : after.ok && after.protected
  return { name, kind: 'teleport', cause, expectedBlocked, before, after, passed }
}

async function runAllowedProbe (bot) {
  await resetEnemy(bot)
  await command(bot, `/surfaceguard allow ${USERNAME} 20`, 50)
  await command(bot, '/surfaceguard probe pearl 1.5 65 0.5', 250)
  const after = await snapshot(bot)
  return {
    name: 'temporary_allow_control',
    kind: 'control',
    expectedBlocked: false,
    after,
    passed: after.ok && after.protected && after.position.x >= 1.0
  }
}

async function runTruceProbe (bot) {
  await resetEnemy(bot)
  await command(bot, `/surfaceguard relation ${USERNAME} truce`, 80)
  await command(bot, '/surfaceguard probe pearl 1.5 65 0.5', 250)
  const after = await snapshot(bot)
  return {
    name: 'truce_control',
...
async function mountOutsideBoat (bot, x = -0.8) {
...
async function runMountedRegression (bot) {
  ...
  const boat = await mountOutsideBoat(bot)
  const before = await snapshot(bot)
  ...
  // Keep emitting stale vehicle-shaped traffic after the server has ejected or
  // corrected the player. The post-transition quarantine must still pin the
  // player to the outside anchor.
  for (let i = 0; i < 40; i++) {
    writeVehicleMove(bot, boat, 1.45 + i * 0.25)
    if ((i + 1) % 10 === 0) await sleep(50)
  }
  await sleep(900)
  const afterStaleFlood = await snapshot(bot)
  const passed = before.ok
    && afterRollback.ok && afterRollback.position.x < 0.0 && !afterRollback.protected
    && afterStaleFlood.ok && afterStaleFlood.position.x < 0.0 && !afterStaleFlood.protected
...
async function buildDismountCage (bot) {
  await command(bot, '/fill -3 64 -3 3 64 3 minecraft:stone')
  await command(bot, '/fill -3 65 -3 3 69 3 minecraft:air')
  // Close the wilderness-side and lateral exits, leaving the protected side as
  // the most attractive vanilla dismount candidate.
...
async function runDismountRegression (bot, boatX) {
  ...
  await teleport(bot, boatX)
  const boat = await mountOutsideBoat(bot, boatX)
  const before = await snapshot(bot)
  ...
  await sleep(700)
  const after = await snapshot(bot)
  const passed = Boolean(boat) && before.ok && after.ok
    && after.position.x < 0.0 && !after.protected && !after.collision
  return {
...
function writeReport (results, bot) {
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, 'surface-guard-report.json'), JSON.stringify(report, null, 2))
  ...
  for (const result of results) {
    ...
    rows.push([
      ...
      result.kind,
      result.passed,
      result.expectedBlocked ?? '',
      after.position?.x?.toFixed(6) ?? '',
      after.protected ?? '',
      after.collision ?? '',
      String(after.vehicle ?? '').replaceAll(',', ';')
    ].join(','))
  }
...
async function shutdown (bot, code) {
  try {
    if (bot && bot._client && !bot._client.ended) bot.quit('SurfaceGuard regression complete')
  } catch (_) {}
  await sleep(250)
  process.exit(code)
}
...
async function main () {
  ...
  bot.on('kicked', reason => console.error('[SurfaceGuard kicked]', reason))
  bot.on('error', error => console.error('[SurfaceGuard error]', error))
  ...
  try {
    ...
    for (const [name, cause] of [
      ...
      const result = await runProbe(bot, name, cause, { x: 1.5, y: 65, z: 0.5 }, true)
      results.push(result)
      writeReport(results, bot)
      console.log(`[SurfaceGuard] ${name} passed=${result.passed} x=${result.after.position?.x}`)
    }

    await command(bot, '/setblock -4 65 0 minecraft:obsidian')
    const collisionResult = await runProbe(
      bot,
      'pearl_solid_overlap',
      'pearl',
      { x: -3.5, y: 65, z: 0.5 },
    ...
    const allowed = await runAllowedProbe(bot)
    results.push(allowed)
    writeReport(results, bot)

    const truce = await runTruceProbe(bot)
    results.push(truce)
    writeReport(results, bot)

    const vehicle = await runMountedRegression(bot)
    results.push(vehicle)
    writeReport(results, bot)

    for (const x of [-0.80, -0.55, -0.30, -0.05]) {
      const dismount = await runDismountRegression(bot, x)
      results.push(dismount)
      writeReport(results, bot)
    }

    const failed = results.filter(result => !result.passed)
    console.log(`[SurfaceGuard] completed=${results.length} passed=${results.length - failed.length} failed=${failed.length}`)
    if (failed.length > 0) {
      console.error('[SurfaceGuard] failed rows', failed.map(result => result.name))
    }
    await shutdown(bot, failed.length === 0 ? 0 : 1)
  } catch (error) {
    console.error('[SurfaceGuard fatal]', error)
