...
const OUTPUT_DIR = path.resolve(process.env.PHASELAB_OUTPUT || 'output-arrow-stack-xp')
...
async function main () {
  ...
  try {
    ...
    report.totalEventGain = report.trials.reduce((sum, trial) => sum + trial.eventGain, 0)
    report.confirmed = report.trials.length === 5 && report.trials.every(trial => trial.exploited)
  } catch (error) {
    report.fatal = String(error.stack || error)
    record('fatal', { error: report.fatal })
  } finally {
    report.finishedAt = new Date().toISOString()
    fs.writeFileSync(path.join(OUTPUT_DIR, 'arrow-stack-xp-report.json'), JSON.stringify(report, null, 2))
    fs.writeFileSync(path.join(OUTPUT_DIR, 'arrow-stack-xp-transcript.jsonl'), transcript.map(row => JSON.stringify(row)).join('\n') + '\n')
    for (const bot of bots) {
      try { bot.quit('PhaseLab stacked arrow XP complete') } catch (_) {}
    }
  }
