...
const report = {
  ...
  scope: 'authorized exact private Sakura/Factions adaptive tick-paced vehicle-only downward phase',
...
async function runTrial (phaseBot, attackerBot, trial, run) {
  ...
  while (acceptedY > TARGET_Y + 1e-9 && attackerBot.vehicle) {
    ...
    const witness = { segment, fromY: acceptedY, targetY, playerAccepted, vehicleAccepted, corrections: corrections.events.length }
    segmentWitnesses.push(witness)
    record('vertical_segment', { trial: trial.id, run, ...witness })
    if (!playerAccepted || !vehicleAccepted || corrections.events.length > 0) break
    acceptedY = targetY
  ...
  record('adaptive_down_trial', result)
  return result
}
...
async function main () {
  ...
  const trialPlan = [
    { id: 'boat-adaptive-down-075', vehicle: 'boat', pulse: 'none', onGround: false, segmentLength: 0.75, pauseMs: 350, repeats: 2 },
    { id: 'horse-adaptive-down-075', vehicle: 'horse', pulse: 'none', onGround: false, segmentLength: 0.75, pauseMs: 350, repeats: 2 }
  ]
  ...
  try {
    ...
    for (const trial of trialPlan) {
      for (let run = 1; run <= trial.repeats; run++) {
        try {
          ...
          record('adaptive_down_trial_error', failure)
        }
      }
    }
  } catch (error) {
    ...
    fs.writeFileSync(path.join(OUTPUT_DIR, 'private-stack-adaptive-down-report.json'), JSON.stringify(report, null, 2))
    fs.writeFileSync(path.join(OUTPUT_DIR, 'private-stack-adaptive-down-transcript.jsonl'), transcript.map(row => JSON.stringify(row)).join('\n') + '\n')
    for (const bot of bots) {
      try { bot.quit('PhaseLab adaptive down matrix complete') } catch {}
    }
  }
