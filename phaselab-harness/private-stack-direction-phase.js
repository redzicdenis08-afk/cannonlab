...
const OUTPUT_DIR = path.resolve(process.env.PHASELAB_OUTPUT || 'output-private-direction')
const Y = 65
const STEP = 0.25
const PAUSE_MS = 100
const THICKNESS = 64
...
const report = {
  ...
  port: PORT,
  version: VERSION,
  scope: 'authorized exact private Sakura/Factions four-direction vehicle phase',
...
async function setupFactions (phaseBot, victimBot, attackerBot) {
  ...
  await command(phaseBot, '/fa power set VictimBot 5000', 500)
...
async function main () {
  ...
  try {
      ...
      for (const directionName of Object.keys(DIRECTIONS)) {
        for (let run = 1; run <= 2; run++) {
          try {
            ...
            record('direction_trial_error', failure)
          }
        }
      }
    }
    ...
    fs.writeFileSync(
      path.join(OUTPUT_DIR, 'private-stack-direction-phase-report.json'),
      JSON.stringify(report, null, 2)
    )
    fs.writeFileSync(
      path.join(OUTPUT_DIR, 'private-stack-direction-phase-transcript.jsonl'),
      transcript.map(row => JSON.stringify(row)).join('\n') + '\n'
    )
    for (const bot of bots) {
      try { bot.quit('PhaseLab direction matrix complete') } catch {}
    }
  }
