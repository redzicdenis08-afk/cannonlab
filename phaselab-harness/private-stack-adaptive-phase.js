...
const OUTPUT_DIR = path.resolve(process.env.PHASELAB_OUTPUT || 'output-private-adaptive')
const Y = 65
const STEP = 0.25
const PAUSE_MS = 350
...
const report = {
  ...
  port: PORT,
  version: VERSION,
  scope: 'authorized exact private Sakura/Factions adaptive tick-paced vehicle-only phase',
...
async function main () {
  ...
  try {
      ...
      for (const directionName of Object.keys(DIRECTIONS)) {
        for (let run = 1; run <= 1; run++) {
          try {
            ...
            record('adaptive_trial_error', failure)
          }
        }
      }
    }
    ...
    fs.writeFileSync(
      path.join(OUTPUT_DIR, 'private-stack-adaptive-phase-report.json'),
      JSON.stringify(report, null, 2)
    )
    fs.writeFileSync(
      path.join(OUTPUT_DIR, 'private-stack-adaptive-phase-transcript.jsonl'),
      transcript.map(row => JSON.stringify(row)).join('\n') + '\n'
    )
    for (const bot of bots) {
      try { bot.quit('PhaseLab adaptive matrix complete') } catch {}
    }
  }
