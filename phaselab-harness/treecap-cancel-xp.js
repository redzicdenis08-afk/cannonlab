...
const OUTPUT_DIR = path.resolve(process.env.PHASELAB_OUTPUT || 'output-treecap-cancel-xp')
...
async function main () {
  ...
  try {
    ...
    for (let run = 1; run <= 3; run++) {
      ...
      const cancelledGain = Number(after.cancelled_events) - Number(before.cancelled_events)
      const trial = {
        run,
        before,
        invoke,
        ...
        xpGain,
        cancelledGain,
        logsUnchanged: Number(after.protected_logs) === Number(before.protected_logs),
        sourceUnchanged: after.source_type === before.source_type && after.source_type === 'OAK_LOG',
        exploited: Boolean(invoke.invoked && xpGain > 0 && cancelledGain > 0 && Number(after.protected_logs) === Number(before.protected_logs))
      }
    ...
    report.totalCancelledEvents = report.trials.reduce((sum, trial) => sum + trial.cancelledGain, 0)
  } catch (error) {
    report.fatal = String(error.stack || error)
    record('fatal', { error: report.fatal })
  } finally {
    report.finishedAt = new Date().toISOString()
    fs.writeFileSync(path.join(OUTPUT_DIR, 'treecap-cancel-xp-report.json'), JSON.stringify(report, null, 2))
    fs.writeFileSync(path.join(OUTPUT_DIR, 'treecap-cancel-xp-transcript.jsonl'), transcript.map(row => JSON.stringify(row)).join('\n') + '\n')
    for (const bot of bots) {
      try { bot.quit('PhaseLab Treecapitator XP complete') } catch (_) {}
    }
  }
