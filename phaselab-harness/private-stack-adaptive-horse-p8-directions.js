...
const OUTPUT_DIR = path.resolve(process.env.PHASELAB_OUTPUT || 'output-private-adaptive-horse-p8-directions')
const Y = 65
const STEP = 0.25
const PAUSE_MS = 350
...
const report = {
  ...
  port: PORT,
  version: VERSION,
  scope: 'authorized exact private Sakura/Factions calibrated horse p8 all-cardinal matrix',
...
async function sendRange (bot, start, direction, fromDistance, toDistance, fixedY, yaw, correctionRef) {
  ...
  while (distance <= toDistance + 1.0E-9 && bot.vehicle && !correctionRef.value) {
    for (let group = 0; group < 8 && distance <= toDistance + 1.0E-9; group++) {
...
async function runTrial (phaseBot, attackerBot, vehicleName, directionName, run) {
  ...
  record('adaptive_horse_p8_direction_trial', result)
  return result
}
...
async function main () {
  ...
  try {
      ...
      for (const directionName of Object.keys(DIRECTIONS)) {
        for (let run = 1; run <= 1; run++) {
          try {
            ...
            record('adaptive_horse_p8_direction_trial_error', failure)
          }
        }
      }
    }
    ...
    fs.writeFileSync(
      path.join(OUTPUT_DIR, 'private-stack-adaptive-horse-p8-directions-report.json'),
      JSON.stringify(report, null, 2)
    )
    fs.writeFileSync(
      path.join(OUTPUT_DIR, 'private-stack-adaptive-horse-p8-directions-transcript.jsonl'),
      transcript.map(row => JSON.stringify(row)).join('\n') + '\n'
    )
    for (const bot of bots) {
      try { bot.quit('PhaseLab adaptive horse p8 directions matrix complete') } catch {}
    }
  }
