...
const OUTPUT_DIR = path.resolve(process.env.PHASELAB_OUTPUT || 'output-private-adaptive-horse-classified')
const Y = 65
const STEP = 0.25
const PAUSE_MS = 350
...
const report = {
  ...
  port: PORT,
  version: VERSION,
  scope: 'authorized exact private Sakura/Factions adaptive horse phase with advanced rider-sync classification',
...
async function sendRange (bot, start, direction, fromDistance, toDistance, fixedY, yaw, correctionRef) {
  ...
  for (let distance = fromDistance + STEP; distance <= toDistance + 1.0E-9 && bot.vehicle && !correctionRef.value; distance += STEP) {
    ...
    // Match the Fabric 4.8 grammar: move only the vehicle client object.
    // The server's passenger handling owns the rider position.
...
async function runTrial (phaseBot, attackerBot, vehicleName, directionName, run) {
  ...
  record('adaptive_horse_classified_trial', result)
  return result
}
...
async function main () {
  ...
  try {
      ...
      for (const directionName of Object.keys(DIRECTIONS)) {
        for (let run = 1; run <= 1; run++) {
          try {
            ...
            record('adaptive_horse_classified_trial_error', failure)
          }
        }
      }
    }
    ...
    fs.writeFileSync(
      path.join(OUTPUT_DIR, 'private-stack-adaptive-horse-classified-report.json'),
      JSON.stringify(report, null, 2)
    )
    fs.writeFileSync(
      path.join(OUTPUT_DIR, 'private-stack-adaptive-horse-classified-transcript.jsonl'),
      transcript.map(row => JSON.stringify(row)).join('\n') + '\n'
    )
    for (const bot of bots) {
      try { bot.quit('PhaseLab adaptive horse classified matrix complete') } catch {}
    }
  }
